...
function useAction (action) {
  ...
  if (actionItem) {
    ...
    console.log(`dynamo-tables:table-session - Did not recognize ${action} action`)
  }
}
...
function analyzeRequest (action, params) {
  ...
  if (action !== 'batchGet' && !params.TableName) {
    console.log(chalk.yellow(`WARNING - dynamo-tables:${TableName} table - TableName param required for request`))
  }

  if ((action === 'scan' || action === 'query') && !params.IndexName) {
    console.log(chalk.yellow(`WARNING - dynamo-tables:${TableName} table - No index key specified for scan or query on ${TableName} table. Optimize this request with an index key`))
  }
}
...
function analyzeResponse (action, params, response) {
  ...
  if (Items && Items > 50 && Object.keys(Items[0]).length > 10) {
    console.log(chalk.yellow(`WARNING - dynamo-tables:${TableName} table - Returning more than 10 keys for per item. Consider using a ProjectionExpression and declare the keys you need.`))
  }
...
function getPolicy () {
  const buildPolicyActions = Array.from(actions).map(action => `"dynamodb:${action}"`)
  const buildPolicyTables = Array.from(tables).map(table => `"arn:aws:dynamodb:us-west-2:TODO_AWS_ACCOUNT_ID:table/*-${table}"`)
