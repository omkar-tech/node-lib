      ...
      docClient[action](params)
        ...
        .then(res => {
          ...
          if (LastEvaluatedKey) {
            ...
            const successMessage = Array.isArray(data) ? data.length + ' records' : '1 record'
...
function getTableName () {
  ...
  if (this.noPrefix) {
    ...
    console.warn(chalk.yellow(`Getting table named '${this.name}' without a stage prefix. This may be an error.`))
    return this.name
  } else {
...
function getById (Id, addParams) {
  ...
  return createRequest('get', params)
...
function getAllById (IdArray) {
  ...
  reqIds.forEach(group => {
    requests.push(createRequest('batchGet', {
      RequestItems: {
        [ this.getTableName() ]: {
          Keys: group
        }
...
function CreateTable (name, options) {
  if (!name) throw new Error('table: createTable requires a name to initialize')
  options = options || {}
